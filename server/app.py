...
One endpoint for now: GET /map?repo=<path>&target=<dir>[&target=…]. The chat and
the agent are later endpoints on the same process.

Parsing a large repo takes minutes, so results are cached per (repo, targets).
The cache remembers which commit the repo was on: same commit → instant answer,
new commit → the pipeline runs again. `refresh=true` forces a re-run regardless.
...
def _run_pipeline(root: Path, targets: list[Path], exts: set[str] | None) -> dict:
...
@app.get("/map")
def get_map(repo: str,
            target: list[str] = Query(default=["src"]),
            exts: str | None = None,
            refresh: bool = False):
    root = Path(repo).expanduser().resolve()
    targets = [root / t for t in target]
    if missing := [t for t in targets if not t.is_dir()]:
        raise HTTPException(404, f"not a directory: {[str(m) for m in missing]}")
    ext_set = ({e if e.startswith(".") else f".{e}" for e in exts.split(",")}
               if exts else None)

    head = _head(root)
    key = sha1("|".join([str(root), *sorted(target), exts or ""]).encode()).hexdigest()[:8]
    ...
    doc = _run_pipeline(root, targets, ext_set)
